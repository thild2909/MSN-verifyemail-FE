 ...
 * endpoint (`/v1/check_email_batch`), with a graceful mock fallback when the
 * backend is unreachable. Shared by the automatic post-collection pass
 * (`company-collect-job`) and the manual "Verify emails" action (API route).
 */
import "server-only";
