...
import { purgeCollectCaches } from "@/server/purge-collect-cache";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const companies = store.rawCompanies(id);
  const cache = await purgeCollectCaches(companies);

  if (!store.deleteCollectJob(id)) {
    return NextResponse.json({ success: false, error: { code: "NOT_FOUND", message: "Collection job not found." } }, { status: 404 });
  }

  return NextResponse.json({ success: true, data: { id, cache } });
}
